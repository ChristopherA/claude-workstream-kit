...
2. **Phases**: every `### <Name> (<XX>)` heading under `## Backlog` -- text after the code, such as a retirement note, is tolerated, since real files carry it -- each with `grep -cE '^ *- \[ \] #<XX>-'` and `grep -cE '^ *- \[ \] #G-<XX>'`. Their sum must equal `grep -cE '^ *- \[ \] #'` for the file; a shortfall is a task sitting outside any phase heading, reported as such.
