...
- Workstream state files: frontmatter is flat `key: value` (read with head); open tasks count with `grep -cE '^ *- \[ \] #'` — anchor on the task-ID form and tolerate leading indentation, as the session-start hook does, since a bare `^- \[ \]` also matches Deletion Criteria and overstates the backlog while a strict anchor drops an indented sub-task; handoff inboxes are `.state/handoffs/*.md`.
