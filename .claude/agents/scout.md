...
- Workstream state files: frontmatter is flat `key: value` (read with head); open tasks count with `grep -cE '^- \[ \] #'` — anchor on the task-ID form, since a bare `^- \[ \]` also matches Deletion Criteria and overstates the backlog; handoff inboxes are `.state/handoffs/*.md`.
